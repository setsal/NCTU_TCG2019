...
class rndenv(random_agent):
    ...
    def mapIndex(self, state):
        self.maxIndex = max(state)
    ...
    def take_action(self, state):
        ...
        if empty:
            ...
            # get max index
            self.mapIndex(state.state)

            """ PJ3 -> bonus tile add """
            if self.total_count > 0 and self.bonus_count / self.total_count <= 1/21 and self.maxIndex >= 7:
                """ generate bonus list """
                bouns_list = list(range(4, self.maxIndex-2))
                
                # random choice bonus bag
                value = random.choices(population=[bouns_list, self.threes_bag],weights=[1/21, 20/21], k=1)
                if value == [bouns_list]:
                    tile = self.choice(bouns_list)
