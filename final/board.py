...
class board:
    ...
    def init_board(self):
        ...
        self.states = (
            {}
        )  # key:move as location on the board, value:player as pieces type
        self.last_move = -1
    ...
    def adjacent_moves(self, move):
        ...
        adjs = []
        if h > 0:
            adjs.append(self.location_to_move((h - 1, w)))
        if h < self.width - 1:
            adjs.append(self.location_to_move((h + 1, w)))
        if w > 0:
            adjs.append(self.location_to_move((h, w - 1)))
        if w < self.width - 1:
            adjs.append(self.location_to_move((h, w + 1)))
        return adjs
